    ...
    <div class='rounded-lg overflow-hidden mb-2 bg-white'>
      ...
      <form action="<?= route('dashboard.links.store') ?>" method="post">
        <div class='flex gap-2'>
            <input name="url" class='border bg-gray-200 border-gray-300 text-gray-900 rounded flex-1 pl-1' type="text" />
            <button type="submit" class='flex items-center px-2 py-1 border border-gray-300 text-gray-900 rounded-md disabled:opacity-50'>
      ...
      <div class="group text-gray-500 bg-white w-full cursor-pointer hover:shadow-lg transition delay-100 duration-200 ease-in-out hover:scale-[1.02] transform mb-1">
        <div class="hover:bg-blue-100 flex flex-row px-3 py-2 justify-between items-center rounded">
          ...
          <div>
            <a href="<?= url("/dashboard/links/delete/{$link->id}") ?>" class="text-gray-500 font-thin group-hover:text-red-500">
              <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
